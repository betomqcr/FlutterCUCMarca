    ...
    return TextButton(
        child: Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                  color: (isEntrada == true) ? Colors.green : Colors.red),
              borderRadius: const BorderRadius.all(Radius.circular(15)),
            ),
            child: Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Icon((isEntrada == true) ? Icons.login : Icons.logout,
                    color: Colors.white),
...
void marcaEntrada(BuildContext context, TextEditingController actividad,
    TextEditingController comentarios) async {
  final marcaProvider = Provider.of<MarcaProvider>(context, listen: false);
  final userProvider = Provider.of<UserProvider>(context, listen: false);

  if (comentarios.text.isEmpty ||
      actividad.text.isEmpty ||
      marcaProvider.areaSelected.id == null) {
    showAlert(
      context: context,
      title: 'Error',
...
void marcaSalida(BuildContext context, TextEditingController actividad,
    TextEditingController comentarios) async {
